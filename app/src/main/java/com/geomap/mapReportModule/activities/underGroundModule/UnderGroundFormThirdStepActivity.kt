...
//                signFaceBitMap =  saveBitmapToJPGImg(signFaceBitMap!!)
    ...
    private fun postUndergroundInsert() {
        if (isSignRoofEdited) {
            binding.tvImgTitle.text = getString(R.string.roof) + imgTitle
            signRoofBitMap = saveBitmapToJPGImg(signRoofBitMap!!)
            signRoof = TypedFile(CONSTANTS.MULTIPART_FORMAT,
                saveBitmapToJPG("RoofImage", signRoofBitMap!!))
        ...
        if (isSignLeftEdited) {
            binding.tvImgTitle.text = getString(R.string.left) + imgTitle
            signLeftBitMap = saveBitmapToJPGImg(signLeftBitMap!!)
            signLeft = TypedFile(CONSTANTS.MULTIPART_FORMAT,
                saveBitmapToJPG("LeftImage", signLeftBitMap!!))
        ...
        if (isSignRightEdited) {
            binding.tvImgTitle.text = getString(R.string.right) + imgTitle
            signRightBitMap = saveBitmapToJPGImg(signRightBitMap!!)
            signRight = TypedFile(CONSTANTS.MULTIPART_FORMAT,
                saveBitmapToJPG("RightImage", signRightBitMap!!))
        ...
        if (isSignFaceEdited) {
            binding.tvImgTitle.text = getString(R.string.face) + imgTitle
            signFaceBitMap = saveBitmapToJPGImg(signFaceBitMap!!)
            signFace = TypedFile(CONSTANTS.MULTIPART_FORMAT,
                saveBitmapToJPG("FaceImage", signFaceBitMap!!))
        ...
        if (isNetworkConnected(ctx)) {
            if (flagUG == "0" || flagUG == "1") {
                ...
                APIClientProfile.apiService!!.postUndergroundInsert(userId, ugDataModel.name,
                    ...
                    convertedFormat(ugDataModel.ugDate, CONSTANTS.SERVER_TIME_FORMAT,
                        CONSTANTS.DATE_MONTH_YEAR_FORMAT_z), ugDataModel.mapSerialNo,
                    ugDataModel.shift, ugDataModel.mappedBy, ugDataModel.scale,
...
//        paint.setShadowLayer(2f, 0f, 1f, getColor(R.color.primary_theme)) // set text width to canvas width minus 16dp padding
        ...
        val textLayout = StaticLayout("  " + binding.tvImgTitle.text, paint, textWidth,
            Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false) // get height of multiline text
...
//        val rectF = RectF(textHeight.toFloat(), textWidth.toFloat(), textHeight.toFloat(),textWidth.toFloat())
    ...
    override fun onBackPressed() {
        ...
        when (i) {
            0 -> {
                signRoofBitMap = binding.drawing.drawingCache.copy(
                    binding.drawing.drawingCache.config, false)
                binding.tvImgTitle.text = getString(R.string.roof) + imgTitle
//                signRoofBitMap = saveBitmapToJPGImg(signRoofBitMap!!)
            ...
            1 -> {
                signLeftBitMap = binding.drawing.drawingCache.copy(
                    binding.drawing.drawingCache.config, false)
                binding.tvImgTitle.text = getString(R.string.left) + imgTitle
//                signLeftBitMap = saveBitmapToJPGImg(signLeftBitMap!!)
...
//                }
            ...
            2 -> {
                signRightBitMap = binding.drawing.drawingCache.copy(
                    binding.drawing.drawingCache.config, false)
                binding.tvImgTitle.text = getString(R.string.right) + imgTitle
//                signRightBitMap = saveBitmapToJPGImg(signRightBitMap!!)
...
//                }
            ...
            3 -> {
                signFaceBitMap = binding.drawing.drawingCache.copy(
                    binding.drawing.drawingCache.config, false)
                binding.tvImgTitle.text = getString(R.string.face) + imgTitle
//                signFaceBitMap = saveBitmapToJPGImg(signFaceBitMap!!)
