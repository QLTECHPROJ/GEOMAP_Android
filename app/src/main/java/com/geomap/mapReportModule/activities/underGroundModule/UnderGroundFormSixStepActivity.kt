...
class UnderGroundFormSixStepActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ctx = this@UnderGroundFormSixStepActivity
        act = this@UnderGroundFormSixStepActivity
...
//        if (flagUG == "1" || flagUG == "2") {
            ...
            if (signRightBitMap != null) {
                callEnable(signRightBitMap!!, "roof")
            } else {
                callDisable("0")
            }
        /*}else {
            if (ugmr.rightImage != null) {
                callEnable(ugmr.rightImage!!, "roof")
            } else {
                callDisable("0")
            }
//        }
*/
    ...
    private fun saveImage(image: Bitmap) {
        ...
        val i = Intent(act, UnderGroundFormSevenStepActivity::class.java)
...
Tap Setting > permission, and turn "Files and media" on.""")
    ...
    override fun onBackPressed() {
        binding.drawing.isDrawingCacheEnabled = true
        if (binding.drawing.drawingCache != null) {
            signRightBitMap = binding.drawing.drawingCache.copy(binding.drawing.drawingCache.config, false)
            ugmr.rightImage = signRightBitMap
        }
        finish()
    }
