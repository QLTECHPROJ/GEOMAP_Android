...
object APIClientProfile {
        ...
        get() = if (field == null) {
            ...
            val restAdapter : RestAdapter =
                RestAdapter.Builder().setLogLevel(RestAdapter.LogLevel.FULL)
                    .setEndpoint(New_BASE_URL).setRequestInterceptor(MyRetrofitInterceptor())
                    .setClient(OkClient(okHttpClient)).setConverter(GsonConverter(Gson())).build()
    ...
    private class MyRetrofitInterceptor : RequestInterceptor {
        ...
        override fun intercept(
            request : RequestFacade) { //            request.addHeader("platform", "Android");
